...
export function getRandomEvent(leafs: Category[]) {
  if (leafs.length === 0) return undefined;
  return leafs[Math.floor(Math.random() * leafs.length)];
}

export function randNumsUniqueToMax(
  max: number,
  reserved: number[],
  N: number
): number[] {
  const allNums = Array.from({ length: max }, (_, i) => i).filter(
    (num) => !reserved.includes(num)
  );

  // ensure there are enough numbers
  if (allNums.length < N)
    throw new Error("not enough available numbers from 0-max");
